...
class ImageValidator:
    ...
    - do not treat explicit safety phrases like 'no text' as forbidden prompt anchors
    - use softer thresholds for easy single-object cards and stricter ones for relation/comparison scenes
    """
    _CJK_RE = re.compile(r"[一-鿿]")
    _SAFETY_CLAUSE_RE = re.compile(
        r"^(?:no\s+|without\s+)(text|letters|numbers|chinese characters|english words|labels?|captions?|signboards?|watermark|logo)",
        flags=re.IGNORECASE,
    )
    ...
    def _count_anchor_hits(self, spec: ImageSpec, semantic_prompt: str) -> tuple[int, int]:
        ...
        for token in list(spec.must_include[:6]):
            ...
            if not any(ch.isalpha() for ch in token_lc):
                continue
            # ignore generic safety/non-semantic tokens if they leaked into include list
            if token_lc in {"text", "letters", "numbers", "label", "caption", "english words", "chinese characters"}:
                continue
            required_checks += 1
    ...
    def _anchor_policy(self, spec: ImageSpec, required_checks: int) -> tuple[int, float]:
        ...
        if render_mode in {"single_object", "object", "single"} or scene_type in {"literal_object", "literal_educational_scene"}:
            return (1 if required_checks >= 1 else 0, 0.25)
        if render_mode in {"attribute_scene", "icon_card", "text_card"} or pos in {"noun", "adjective"}:
            return (1 if required_checks >= 1 else 0, 0.30)
        if render_mode in {"action_scene", "routine_scene"} or pos in {"verb", "phrase"}:
            return (min(2, required_checks), 0.40)
        if render_mode in {"relation_scene", "contrast_pair", "comparison_scene"} or pos in {"preposition"}:
            return (min(2, required_checks), 0.45)
        return (1 if required_checks >= 1 else 0, 0.34)
    @staticmethod
